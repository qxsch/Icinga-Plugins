...
class DnsResolver {
	protected $nsCache=array();

	protected static $rootServers=array(
	...
	protected function getLastNameServersAsArray(DNSQuery $query) {
		$array=array();
		if(
			$query->lastnameservers===null || 
			$query->lastadditional===null ||
			empty($query->lastnameservers->results) || 
			empty($query->lastadditional->results)
		) {
			return array();
		}
		foreach($query->lastnameservers->results as $result) {
			$array[strtolower($result->data)]=null;
		}
		foreach($query->lastadditional->results as $result) {
			if($result->typeid=='A') {
				$key=strtolower($result->domain);
				if(array_key_exists($key, $array)) {
					$array[$key]=$result->data;
				}
			}
		}
	...
	protected function lookUpAllNS($domain, $udp=true, $crossLookup=false) {
		$domainReverseArray=array_reverse(explode('.', $domain));

		$glueRecords=array();
		$glueRecords[]=array('domain' => '.', 'glue' => self::$rootServers);

		$nsServers=self::$rootServers;
		$currentPath='';
		foreach($domainReverseArray as $currentDomain) {
			$currentPath=strtolower($currentDomain).'.'.$currentPath;
			if(isset($this->nsCache[$currentPath])) {
				$nsServers=$this->nsCache[$currentPath]['glue'];
				$glueRecords[]=$this->nsCache[$currentPath];
			}
			else {
				// query dns servers
				while(!empty($nsServers)) {
					$nsServerName=array_rand($nsServers);
					$nsServerIp=$nsServers[$nsServerName];
					unset($nsServers[$nsServerName]);

					
					$query=new DNSQuery($nsServerIp, 53, $this->timeout, (bool)$udp, false, false);
					$result=$query->Query($currentPath, 'NS');
					// no new nameservers found?
					if(
						$query->lastnameservers===null ||
						$query->lastadditional===null ||
						empty($query->lastnameservers->results) ||
						...
						if($crossLookup && !($query->lastnameservers===null || empty($query->lastnameservers->results))) {
							$nsServers=array();
							foreach($query->lastnameservers->results as $ns) {
								$nsName=strtolower($ns->data);
								$nsServers[$nsName]=gethostbyname($nsName);
							}
							// add to our stack
							$glueRecords[]=array('domain' => $currentPath, 'glue' => $nsServers);
							// add to our stack
							$this->nsCache[$nsName]=$nsServers;
							break;
						}
						continue;
					}
					$nsServers=$this->getLastNameServersAsArray($query);
					// add to our stack
					$glueRecords[]=array('domain' => $currentPath, 'glue' => $nsServers);
					// populate the yycache
					$this->nsCache[$currentPath]=array('domain' => $currentPath, 'glue' => $nsServers);
	...
	public function lookUpAllGlueRecords($domain, $udp=true) {
		return $this->lookUpAllNS($domain, $udp, false);
	}

	public function lookUpRecords($domain, $type, $udp=true) {
		$domainReverseArray=array_reverse(explode('.', $domain));
		$record=array_pop($domainReverseArray);

		$records=array();

		$nsServers=array_pop($this->lookUpAllNS(implode('.', array_reverse($domainReverseArray)), $udp, true));
		$nsServers=$nsServers['glue'];
		// query dns servers
		while(!empty($nsServers)) {
			$nsServerName=array_rand($nsServers);
			$nsServerIp=$nsServers[$nsServerName];
	...
	public function lookUpLastGlueRecords($domain, $udp=true) {
		return array_pop($this->lookUpAllGlueRecords($domain, $udp));
	}

	public function compareGlueRecordsWithDns($domain, $udp=true) {
		// get glue records
		// do we have glue recordsa?
		// compare them with real values
	}
}
